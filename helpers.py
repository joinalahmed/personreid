...
def save_roi(img, count, id):
    base_dir = "/home/leadics-18-2/Desktop/person_tracker_exp/results"

    if not os.path.exists(base_dir):
        os.makedirs(base_dir)

    new_dir = os.path.join(base_dir, 'Track_%s' % (id))

    if not os.path.exists(new_dir):
        os.makedirs(new_dir)

    cv2.imwrite(os.path.join(new_dir, 'image_%s.jpg' % count), img)


def draw_box_label(id, img, bbox_cv2, count, box_color=(0, 255, 255), show_label=False):
    '''
